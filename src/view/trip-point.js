...
const getTimeDuration = (start, finish) => {
  const finishToDate = dayjs(finish);
  const startToDate = dayjs(start);
  const minuteDuration = finishToDate.diff(startToDate, `minute`);
...
const createTripPointTemplate = (point) => {
  ...
  return `<div class="event">
      ...
      ${activeOffers.length !== 0 ? getOffersTemplate(activeOffers) : ``}
